...
class MultiCoreSolrRecipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        self.options["customTemplate"] = str(
            "schema-template" in self.options_orig
        ).lower()
        ...
        sd(
            "log4j-template",
            os.path.realpath(join(self.template_dir, "..", "log4j.properties.tmpl")),
        )
        sd(
            "logging-template",
            os.path.realpath(join(self.template_dir, "..", "logging.properties.tmpl")),
        )
        ...
        sd(
            "stopwords-template",
            os.path.realpath(join(self.template_dir, "..", "stopwords.txt.tmpl")),
        )

        # Synonyms
        sd(
            "synonyms-template",
            os.path.realpath(join(self.template_dir, "..", "synonyms.txt.tmpl")),
        )
    ...
    def solr_autoinstall(self):
        ...
        directory = os.path.join(self.buildout["buildout"]["parts-directory"], name)
    ...
    def get_analyzers(self, options):
        ...
        for analyzer_type in ("query", "index"):
            ...
            for processor in analyzers[analyzer_type]:
                ...
                analyzers[analyzer_type][processor] = self.parse_processor(
                    analyzers[analyzer_type][processor], unique=unique
                )
        return analyzers
    ...
    def java_opts(self):
        ...
        _default_options = [
            (
                ...
                "file://{0}/log4j.properties".format(self.options["jetty-destination"]),
            )
        ]
    ...
    def get_indexes(self, options):  # NOQA
        ...
        for line in options.get("index", "").strip().splitlines():
            ...
            for key in INDEX_ATTRIBUTES:
                ...
                if key == "copyfield":
                    entry[key] = [
                        {"source": entry["name"], "dest": val} for val in value
                    ]
                elif key in ("name", "extras", "default", "type"):
                    entry[key] = value
                elif key in ("omitnorms", "storeOffsetsWithPositions") and not value:
        ...
        if (
            unique
            and not indeces[names.index(unique)].get("required", None) == "true"
        ...
        if default and default not in names:
            raise zc.buildout.UserError(
                "Default search field without matching index: " "{0:s}".format(default)
            )
    ...
    def artifact_prefix(self):
        test_artifacts = os.path.join(self.options["solr-location"], "dist", "apache-*")
    ...
    def copy_files(self, src_glob, dst_folder):
        for fname in glob.iglob(src_glob):
            try:
                ...
                self.logger.error(
                    "Could not copy {0:s} to {1:s}: {2:s}".format(fname, dst_folder, e)
                )
    ...
    def cores(self):
        ...
        if not cores:
            ...
            self.logger.info("No cores option defined. Using %s.", STANDARD_CORE_NAME)
    ...
    def install_base(self):
        ...
        self.copy_solr(self.instance_dir, self.install_dir)
    ...
    def install_core(self, name, options):
        ...
        legacy_options["additionalSchemaConfig"] = options["additional-schema-config"]
        ...
        legacy_options["defaultSearchField"] = options["default-search-field"]

        # Config
        self._generate_from_template(
            default_handler_components=self.get_default_handler_components(options),
            additional_solrconfig=options["additional-solrconfig"],
            additional_solrconfig_query=options["additional-solrconfig-query"],
        ...
        for global_extra_libs_folder in self.options["global-extra-libs"].split("\n"):
